...
from src.recommendation_system.RealEstateInvestmentModel import RealEstateInvestmentModel
...
TOP_OPPORTUNITIES = os.path.join(
    REPORT_DIR, "top_investment_opportunities.csv")
...
def evaluate_predictions(y_true, y_pred, title):
    ...
    print(
        f"  Mean Squared Error (MSE):     {mse:.2f} (Normalized: {norm_mse:.4f})")
    print(
        f"  Root Mean Squared Error:      {rmse:.2f} (Normalized: {norm_rmse:.4f})")
    print(
        f"  Mean Absolute Error (MAE):    {mae:.2f} (Normalized: {norm_mae:.4f})")
    print(
        f"  Median Absolute Error:        {median_abs_error:.2f} (Normalized: {norm_median_error:.4f})")
    print(f"  Mean Absolute % Error (MAPE): {mape:.2f}%")
    print(f"  R² Score:                     {r2:.4f}")
    print(
        f"  Maximum Error:                {max_error:.2f} (Normalized: {norm_max_error:.4f})")
...
def plot_prediction_performance(df, target_col, pred_col, title, output_path):
    ...
    theoretical_quantiles = np.quantile(
        np.random.normal(0, 1, 1000), quantiles)
...
def identify_top_opportunities(df, n=20):
    ...
    top_props = df.sort_values(
        'InvestmentScore', ascending=False).head(n).copy()

    # Add opportunity classification
    conditions = [
        (top_props['ValScore'] > top_props['YieldScore']) & (
            top_props['ValScore'] > top_props['RentScore']),
        (top_props['YieldScore'] > top_props['ValScore']) & (
            top_props['YieldScore'] > top_props['RentScore']),
        (top_props['RentScore'] > top_props['ValScore']) & (
            top_props['RentScore'] > top_props['YieldScore'])
    ]
    choices = ['Undervalued', 'High Yield', 'Rental Upside']
    top_props['Opportunity_Type'] = np.select(
...
def visualize_investment_opportunities(df, area_scores_df, output_dir):
    ...
    top_areas = area_scores_df.sort_values(
        'InvestmentScore', ascending=False).head(15)
    ...
    top_10_areas = area_scores_df.sort_values(
        'InvestmentScore', ascending=False).head(10)
...
def main():
    ...
    logger.info(
        f"Loaded {len(investment_data)} records with {investment_data.shape[1]} features")
    ...
    pd.DataFrame(eval_metrics).to_csv(
        os.path.join(EVAL_DIR, 'model_metrics.csv'))
    logger.info(
        f"Evaluation metrics saved to {os.path.join(EVAL_DIR, 'model_metrics.csv')}")
    ...
    sample_scores = property_scores.sort_values(
    ...
    for _, row in sample_scores.iterrows():
        ...
        print(
            f"    Value Diff: {row['ValDiffPct']:.1f}%, Rental Yield: {row['RentalYield']:.2f}%")
        print()
    ...
    logger.info(
        f"Top investment opportunities exported to {TOP_OPPORTUNITIES}")
    ...
    for i, (_, row) in enumerate(top_opportunities.head(5).iterrows()):
        ...
        if row['Opportunity_Type'] == 'Undervalued':
            print(
                f"     Property appears undervalued by {row['ValDiffPct']:.1f}%")
        elif row['Opportunity_Type'] == 'High Yield':
            print(
                f"     Offers strong rental yield of {row['RentalYield']:.2f}%")
        elif row['Opportunity_Type'] == 'Rental Upside':
            print(f"     Rental upside potential of {row['RentDiffPct']:.1f}%")
        print()

    logger.info(
        "Investment model training and evaluation pipeline completed successfully!")
