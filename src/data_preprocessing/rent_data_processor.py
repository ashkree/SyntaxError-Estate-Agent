...
class RentalDataProcessor:
    ...
    def clean_data(self):
        ...
        try:
            ...
            try:
                ...
                df['contract_end_date'] = pd.to_datetime(
                    df['contract_end_date'], format='%d-%m-%Y', errors='coerce')
        except Exception as e:
        ...
        df['contract_duration_days'] = (
            df['contract_end_date'] - df['contract_start_date']).dt.days

        # Remove contracts with invalid duration (negative or very short/long)
        invalid_duration = (df['contract_duration_days'] < 1) | (
            df['contract_duration_days'] > 1825)  # Max 5 years
        ...
        df['contract_duration_months'] = df['contract_duration_days'] / \
            30.44  # Average days per month
        df['monthly_rent'] = df['contract_amount'] / \
        ...
        rent_outliers = (df['monthly_rent'] < lower_bound) | (
            df['monthly_rent'] > upper_bound)
    ...
    def _update_data_summary(self):
        ...
        self.data_summary['property_types'] = df['ejari_property_type_en'].value_counts(
        ).to_dict()
        self.data_summary['property_usage'] = df['property_usage_en'].value_counts(
        ).to_dict()
...
if __name__ == "__main__":
    ...
    path = "data/training_data/rent_data/rent_data.csv"

    processor = RentalDataProcessor()

    processor.load_data(path)
    processor.clean_data()

    processor.export_processed_data(
        "data/training_data/rental_data/processed_rental_data.csv")
